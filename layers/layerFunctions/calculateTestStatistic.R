...
getTestStatistic <- function(nlsCoupon){ 
    ...
    P <- mvspec(detrendedData, kernel('daniell',2), log='no', plot = FALSE)
    ...
    baselineU <- baseline$fitted.values + zB*SE
    
    # find peaks in periodogram
    peaks <- which(lowerBound > baselineU)
